...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login(token, response.data.email, returnedRole);

      // Redirect based on role
      if (returnedRole.toUpperCase() === 'RECRUITER') {
      ...
      console.error(err);
      setError(err.response?.data || 'Failed to register user. Email may be already taken.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1">
          ...
          <div className="relative">
            <Award className="absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-zinc-500" />
            <select
              value={role}
              onChange={(e) => setRole(e.target.value)}
              className="w-full rounded-xl border border-zinc-900 light:border-zinc-200 bg-zinc-950 light:bg-zinc-50 py-3 pl-10 pr-4 text-sm text-white light:text-black focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 transition-all appearance-none cursor-pointer"
            >
